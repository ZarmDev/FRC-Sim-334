...
//    try {
//        auto rxArray = json::parse(message);
//
//        for (const auto& msg : rxArray) {
//            if (!msg.is_object()) {
//                std::cout << "[NT4] Ignoring text message, JSON parsing did not produce an object." << std::endl;
//                continue;
//            }
//
//            if (!msg.contains("method") || !msg.contains("params")) {
//                std::cout << "[NT4] Ignoring text message, JSON parsing did not find all required fields." << std::endl;
//                continue;
//            }
//
...
//
//            if (!params.is_object()) {
//                std::cout << "[NT4] Ignoring text message, JSON parsing found \"params\", but it wasn't an object." << std::endl;
//                continue;
//            }
//
//            if (method == "announce") {
//                // Handle announce method
//                std::cout << "[NT4] Announce: " << params.dump() << std::endl;
//            }
//            else if (method == "unannounce") {
//                // Handle unannounce method
//                std::cout << "[NT4] Unannounce: " << params.dump() << std::endl;
//            }
//            else if (method == "properties") {
//                // Handle properties method
//                std::cout << "[NT4] Properties: " << params.dump() << std::endl;
//            }
//            else {
//                std::cout << "[NT4] Ignoring text message - unknown method " << method << std::endl;
//            }
//        }
//    }
//    catch (const std::exception& ex) {
//        std::cerr << "[NT4] JSON parsing error: " << ex.what() << std::endl;
//    }
//}

//void handleMsgPackMessage(const std::vector<uint8_t>& data) {
//    try {
//        msgpack::object_handle oh = msgpack::unpack(reinterpret_cast<const char*>(data.data()), data.size());
//        msgpack::object obj = oh.get();
//        std::vector<msgpack::object> rxArray;
//        obj.convert(rxArray);
//
//        for (const auto& unpackedData : rxArray) {
//            int topicID = unpackedData.via.array.ptr[0].as<int>();
//            int64_t timestamp_us = unpackedData.via.array.ptr[1].as<int64_t>();
//            int typeIdx = unpackedData.via.array.ptr[2].as<int>();
//            auto value = unpackedData.via.array.ptr[3];
//
//            if (topicID >= 0) {
//                // Handle new topic data
//                std::cout << "[NT4] New topic data: " << topicID << ", " << timestamp_us << ", " << typeIdx << ", " << value << std::endl;
//            }
//            else if (topicID == -1) {
//                // Handle receive timestamp
//                std::cout << "[NT4] Receive timestamp: " << timestamp_us << ", " << value << std::endl;
//            }
//            else {
//                std::cout << "[NT4] Ignoring binary data - invalid topic id " << topicID << std::endl;
//            }
//        }
//    }
//    catch (const std::exception& ex) {
//        std::cerr << "[NT4] MessagePack parsing error: " << ex.what() << std::endl;
//    }
//}
